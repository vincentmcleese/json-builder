...
export default function WorkflowPage() {
  ...
  const loadWorkflow = async () => {
    ...
    try {
      ...
      // Try to load by slug first since most URLs will be using slugs
      ...
      // If no data found by slug and the slug looks like a UUID, try loading by ID
      if (!data && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(slug)) {
        ...
        if (idError) {
          console.warn('Failed to load workflow by ID:', idError);
        } else {
          data = idData;
        }
      }
      ...
      // If we found it by ID and it has a slug, redirect to the slug URL
